...
def load_data(files_bytes: dict) -> dict:
    frames = {}
    temp_dict = {
        "roster": [], "nbaleaguesumary": [], "def": [], 
        "players": [], "impact": [], "trends": []
    }
...
def calculate_absence_impact(team_name: str, out_players: list, impact_df: pd.DataFrame) -> float:
    ...
    if p_col and diff_col:
        for player in out_players:
            mask = impact_df[p_col].astype(str).str.contains(player, na=False)
            val = impact_df[mask][diff_col]
            if not val.empty:
...
def project_game(home: str, away: str, frames: dict, h_impact: float, a_impact: float) -> dict:
    ...
    return {
        ...
        "spread": spread, "winner": winner, "margin": win_margin
    }
...
def generate_roster_matrix(team: str, opp: str, frames: dict, out_players: list) -> pd.DataFrame:
    ...
    for _, row in roster.iterrows():
        ...
        data.append({
            "Jugador": pname,
            "Pos": pos,
            "PTS": round(pts_val * dvp, 1),
            "REB": round(reb_val * dvp, 1),
            "AST": round(ast_val * dvp, 1),
...
# TAB 1: GAME HUNTER
with tab1:
    st.subheader("Proyección + Ajuste de Bajas")
    ...
    if st.button("Proyectar Partido", type="primary"):
        ...
        if np.isnan(g.get("total", np.nan)):
            ...
            with c1:
                st.metric(label="Ganador", value=g["winner"].upper(), delta=f"Margin: {g['margin']:.1f} pts")
            with c2:
            ...
            if out_home or out_away:
                st.caption(f"Ajuste Lesiones aplicado: {t_home} ({h_impact:+.1f} pts) | {t_away} ({a_impact:+.1f} pts)")
